...
class weather:
    def __init__(self, location):
        ...
        try:
            xml = urllib2.urlopen(
                'http://www.google.com/ig/api?hl=zh-cn&weather=%s'
                % (location)).read().decode('gb2312').encode('utf-8')
        except ValueError:
    ...
    def get_courent_weather(self):
        ...
        self.weather_data['today']['condition'] = dom.getElementsByTagName(
            'condition')[0].attributes['data'].value
        self.weather_data['today']['temp_c'] = dom.getElementsByTagName(
            'temp_c')[0].attributes['data'].value
        self.weather_data['today']['humidity'] = dom.getElementsByTagName(
            'humidity')[0].attributes['data'].value
        self.weather_data['today']['wind'] = dom.getElementsByTagName(
    ...
    def get_forecast_weather(self):
        ...
        for dom in doms:
            ...
            bufdict['day_of_week'] = dom.getElementsByTagName(
                'day_of_week')[0].attributes['data'].value
            bufdict['low'] = dom.getElementsByTagName(
                'low')[0].attributes['data'].value
            bufdict['high'] = dom.getElementsByTagName(
                'high')[0].attributes['data'].value
            bufdict['condition'] = dom.getElementsByTagName(
                'condition')[0].attributes['data'].value
    ...
    def get_data_string(self):
        return self.get_courent_weather_string() + \
            self.get_forecast_weather_string()
